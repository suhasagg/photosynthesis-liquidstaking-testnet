	...
	for _, txGasTrackingInfo := range blockGasTrackingInfo.Txs {
		...
		for _, contractOp := range txGasTrackingInfo.ContractOperations {
			...
			if !opEligible {
				...
				k.Logger(ctx).Info("Noop contract operation found (skip)", "txID", contractOp.TxId, "opID", contractOp.Id)
				continue
			}
...
func (k Keeper) estimateBlockRewards(ctx sdk.Context, blockDistrState *blockRewardsDistributionState) *blockRewardsDistributionState {
	...
	if found && blockRewards.HasRewards() {
		...
		k.Logger(ctx).Info("No inflation rewards to distribute (no record / empty coin / gas limit not set)", "height", blockDistrState.Height)
	}
	k.Logger(ctx).Info("Inflation rewards - block", blockRewards.InflationRewards)
	k.Logger(ctx).Info("Inflation rewards - block", "rewards", blockRewards.InflationRewards.Amount)
	...
	for txID := range blockDistrState.Txs {
		...
		if found && txRewards.HasRewards() {
			...
			k.Logger(ctx).Info("Tx rewards - Tx", txRewards.FeeRewards)
			k.Logger(ctx).Info("Tx rewards - block", "rewards", txRewards.FeeRewards)
		} else {
			k.Logger(ctx).Info("No tx fee rebate rewards to distribute (no record / empty coins)", "txID", txID)
		}
	}
...
func (k Keeper) createRewardsRecords(ctx sdk.Context, blockDistrState *blockRewardsDistributionState) {
	...
	for _, contractDistrState := range blockDistrState.Contracts {
		...
		if contractDistrState.FeeRewards.IsZero() && contractDistrState.InflationaryRewards.IsZero() {
			k.Logger(ctx).Info("No contract rewards to distribute (skip)", "contract", contractDistrState.ContractAddress)
			continue
		}
		if contractDistrState.Metadata == nil {
			k.Logger(ctx).Info("Contract metadata is not set (skip)", "contract", contractDistrState.ContractAddress)
			continue
		}
		if !contractDistrState.Metadata.HasRewardsAddress() {
			k.Logger(ctx).Info("Contract rewards address is not set (skip)", "contract", contractDistrState.ContractAddress)
			continue
		}
	...
	for _, contractDistrState := range contractStates {
		...
		k.Logger(ctx).Info("Contract Inflationary rewards", contractDistrState.InflationaryRewards)
		k.Logger(ctx).Info("Contract Inflationary rewards", "rewards", contractDistrState.InflationaryRewards.Amount)
		k.Logger(ctx).Info("Contract Fee rewards", contractDistrState.FeeRewards)
		k.Logger(ctx).Info("Contract Fee rewards", "rewards", contractDistrState.FeeRewards.AmountOf("stake"))
		// Create a new record
