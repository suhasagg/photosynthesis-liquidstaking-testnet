...
if [[ "${ALL_HOST_CHAINS:-false}" == "true" ]]; then 
  HOST_CHAINS=(GAIA EVMOS PHOTO HOST)
...
IBC_PHOTO_CHANNEL_0_DENOM='ibc/622C9E60DF523F11A01E78C105756CE51729296F483B4609DABC060A2CA148EA'
IBC_PHOTO_CHANNEL_1_DENOM='ibc/EAEB74E11A7BFFC17E79B802EF01D3307A285F7AA802745037B021609ECFB034'
IBC_PHOTO_CHANNEL_2_DENOM='ibc/9117A26BA81E29FA4F78F57DC2BD90CD3D26848101BA880445F119B22A1E254E'
IBC_PHOTO_CHANNEL_3_DENOM='ibc/A4DB47A9D3CF9A068D454513891B526702455D3EF08FB9EB558C561F9DC2B701'
...
IBC_PHOTO_DENOM='ibc/622C9E60DF523F11A01E78C105756CE51729296F483B4609DABC060A2CA148EA'
IBC_JUNO_DENOM=$IBC_JUNO_CHANNEL_2_DENOM
IBC_OSMO_DENOM=$IBC_OSMO_CHANNEL_3_DENOM
#IBC_STARS_DENOM=$IBC_STARS_CHANNEL_4_DENOM
IBC_STPHOTO_DENOM='ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145'
# CHAIN PARAMS
BLOCK_TIME='1s'
...
# PHOTO
PHOTO_CHAIN_ID=localnet
PHOTO_NODE_PREFIX=photo
PHOTO_NUM_NODES=1
PHOTO_BINARY="$DOCKERNET_HOME/../build/archwayd"
PHOTO_VAL_PREFIX=pval
PHOTO_REV_ACCT=prev1
PHOTO_ADDRESS_PREFIX=archway
PHOTO_DENOM=$PHOTO_DENOM
PHOTO_RPC_PORT=26457
PHOTO_MAIN_CMD="$PHOTO_BINARY --home $DOCKERNET_HOME/state/${PHOTO_NODE_PREFIX}1"
PHOTO_RECEIVER_ADDRESS='archway1n3fvgm3ck5wylx6q4tsywglg82vxflj3h8e90m'
...
RELAYER_PHOTO_EXEC="$DOCKER_COMPOSE run --rm relayer-photo"
...
RELAYER_STRIDE_MNEMONIC1="renew umbrella teach spoon have razor knee sock divert inner nut between immense library inhale dog truly return run remain dune virus diamond clinic"
...
RELAYER_PHOTO_MNEMONIC="grit forum glass purchase pattern season priority flight write young net ice album awful clog enhance direct nasty siren awake warrior unfold bless switch"
RELAYER_MNEMONICS=(
  "$RELAYER_GAIA_MNEMONIC"
  "$RELAYER_PHOTO_MNEMONIC"
...
PHOTO_ADDRESS() {
  $PHOTO_MAIN_CMD keys show ${PHOTO_VAL_PREFIX}1 --keyring-backend test -a
}
...
GET_VAL_ADDR() {
  ...
  MAIN_CMD=$(GET_VAR_VALUE ${chain}_MAIN_CMD)
  output=$($MAIN_CMD q staking validators | grep ${chain}_${val_index} -A 5 | grep operator | awk '{print $2}')
  echo $output
  #$MAIN_CMD q staking validators | grep ${chain}_${val_index} -A 5 | grep operator | awk '{print $2}'
  $MAIN_CMD q staking validators | grep -oP 'operator_address:\s*\K[^ ]*' | tr -d '\n' |  sed -e 's/^[ \t\n]*//' -e 's/[ \t\n]*$//'
}
